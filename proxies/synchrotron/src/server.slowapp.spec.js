...
describe('express with slowapp no insecure', function () {
    ...
    before(function () {
        ...
        app.setup({UPSTREAM: "https://localhost:9003", ALLOW_INSECURE: "false"});
        server = app.server;
    });
...
describe('express with slowap', function () {
    ...
    before(function () {
        env = process.env;
        let app = require('./server');
        app.setup({UPSTREAM: "https://localhost:9003", ALLOW_INSECURE: "true"});
        server = app.server;
    });
    ...
    it('it times out if syncwait shorter than initial response', (done) => {
        request(server)
            .get('/sync/slow?delay=5&syncWait=0.25')
    ...
    it('slowapp default syncWait final status 500', (done) => {
        request(server)
            .get('/sync/slow?final_status=500&complete_in=0.5')
            .set('Accept', 'application/json')
            .expect(res => {
    ...
    it('slowapp default syncWait not complete', (done) => {
        request(server)
            .get('/sync/slow?final_status=500&complete_in=5&syncWait=1')
